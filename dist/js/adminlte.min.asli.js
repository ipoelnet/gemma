...
function(a) {
    ...
    a.fn.tree = b, a.fn.tree.Constructor = h, a.fn.tree.noConflict = function() {
		...
		uOrigin		= window.location.origin,
		uPathname	= '/';
		a('ul.sidebar-menu')
		.find('a')
		.each(function(){
			var _		= a(this),
				href	= _.attr('href');
				href 	= href.replace('../',uOrigin+uPathname+'assets/pages/');
				_.attr('href',href);
		});
		
		a('[href="https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,600,700,300italic,400italic,600italic"]').remove();
		a('head').append('<link href="'+uOrigin+uPathname+'assets/source-sans-pro-v11-latin/source-sans-pro.css" rel="stylesheet" type="text/css">');
		a('head').append('<link rel="canonical shortcur icon" href="'+uOrigin+uPathname+'assets/images/logo-64.png"/>');
		a('head title').text('SIM Yayasan Darut Taqwa Sengonagung');
		var aLogo	= a('header.main-header a.logo'),
			rLogo	= '<span class="logo-mini" style="background: url('+uOrigin+uPathname+'assets/images/logo-32.png) center center no-repeat;height: -webkit-fill-available;"></span><span class="logo-lg" style="background: url('+uOrigin+uPathname+'assets/images/logo-text.png) center center no-repeat;height: -webkit-fill-available;"></span>';
		
		aLogo.attr('href',uOrigin+uPathname);
		aLogo.html(rLogo);
		if(aLogo.attr('href')==url){
