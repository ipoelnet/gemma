...
function(a) {
    ...
    h.prototype.toggle = function() {
        ...
        a(this.element).on("click", this.options.collapseTrigger, function(c) {
        ...
        }), a(this.element).on("click", this.options.pageInfo, function(c) {
			var ini = a(this);
			...
			$.confirm({
				...
				content: function () {
					var self = this;
					return $.ajax({
						url		: apps_config.domain+'/info',
						dataType: 'json',
						method	: 'post',
						data	: {data:ini.attr('data-info')}
					}).done(function (response) {
						self.setContent(response.data);
						/* self.setContentAppend('<br>Version: ' + response.data); */
						self.setTitle('Informasi Halaman');
					}).fail(function(){
						self.setContent('Tidak ada info data');
					});
				},
				buttons: {
					tutup: {
...
function(a) {
    ...
    var c = "lte.layout",
        ...
        e = {
            ...
            h2ndHeader: "header.h2nd-header",
            h3ndHeader: "header.h3nd-header",
            sidebar: ".sidebar",
    ...
    g.prototype.activate = function() {
        ...
        if (a("body").hasClass(f.fixed)) a(e.contentWrapper).css("min-height", d - b);
			...
			if(a("body").hasClass('layout-top-nav')){
				var h2 = isNaN(a(e.h2ndHeader).height()) ? 0:a(e.h2ndHeader).height(),
					h3 = isNaN(a(e.h3ndHeader).height()) ? 0:a(e.h3ndHeader).height();
				var he = d-c-h2-h3-1;
				a(e.contentWrapper).css({"min-height":he,"max-height": he,'overflow-y':'scroll'})};
...
function(a) {
	...
	a.fn.menu_link=function(){
		var ini		= a(this),
		uiGet	= ini.attr('ui-get'),
		uiJp	= ini.attr('ui-jp'),
		href	= ini.attr('href') || "",
		...
		uPathname	= '/';
		if(uiGet){
			//if(uOrigin.indexOf('sikeu.daruttaqwa') || uOrigin.indexOf('sisda.daruttaqwa') || uOrigin.indexOf('simpeg.kanjabung')){
			ini.click(function(e){
				e.preventDefault();
				e.stopPropagation();
				e.stopImmediatePropagation();

				if(iLS.get('active-menu')==uiJp){return};
				// jika menu berupa dropdown
				var dropdowns = ini.closest('li.dropdown').find('[data-toggle="dropdown"]');
				if(dropdowns){
					dropdowns.dropdown("toggle");
				};
				var ajax = a("body").SimpleAjax({
					url_full : uiGet,
					waitme	: true,
					success	: function(data){
						if(data){
							a('[ui-content]').html(data);
							iLS.set('active-menu',uiJp);
						}else if(!data || data.info){
							Log.log('err');
						};
						a(window).trigger('load');
					}
				});
				ajax.always(function(data){
					var self = $(data),
						options = eval("[" + ini.attr("ui-options") + "]");
						a.isPlainObject(options[0]) && (options[0] = $.extend({}, options[0]));
					uiLoad.load(MODULE_SWN[uiJp]).then(function () {
						self[uiJp].apply(self, options);
						$.tooltips();
					});
				});
				ajax.fail(function (jqXHR, textStatus, errorThrown){
					a.Snackbar({
						text : "Mohon maaf, ada permasalahan koneksi jaringan"
					});
				});
				//return false;
			});
		...
		// set active menu null
		iLS.set('active-menu',null);
		
        var url = window.location.href,
		uOrigin		= window.location.origin,
		uPathname	= '/',
		uMenu		= a('ul.sidebar-menu');
		if(a('body').hasClass('layout-top-nav')){
			uMenu = a('#navbar-collapse');
		};
		uMenu.find('a')
		.each(function(){
			a(this).menu_link()
		...
		if(url.indexOf('assets/pages')){
			a('head').append('<link href="'+uOrigin+uPathname+'assets/bower_components/bootstrap/dist/css/glyphicon.css" rel="stylesheet" type="text/css">');
		};
			...
			a(this).SimpleAjax({
					...
					success	: function(data){
						if(data.success){
							...
							a.Alert({
								content : 'Gagal Logout'
							});
						}
					}
				});
		});
		a('.btn_password').click(function(){
			a(this).SimpleAjax({
					waitme 	: true,
					url		: '/login/password_form',
					data	: {},
					headers	: {log:true},
					error_alert : true,
					success	: function(data){
		...
		//layout-top-nav
		function deploysdata(){
			setTimeout(function(){
				Log.clear();
				$('body').attr('token',sha256(1024)+':1024');
				Log.log(sha256(1024)+':1024')
				deploysdata();
			}, 10000);
		};
		//deploysdata();
